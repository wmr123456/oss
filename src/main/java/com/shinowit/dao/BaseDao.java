...
public class BaseDao<T> {
    ...
    public List<T> queryByHql(String hql, final Object... objects) {
        List<T> list = null;
        Session session = null;
        try {
            session = sessionFactory.getCurrentSession();
    ...
    public boolean executeByHql(String hql, String... objectses) {
        boolean result = true;
        Session sesssion = null;
        try {
            sesssion = sessionFactory.getCurrentSession();
            Query query = sesssion.createQuery(hql);
            for (int i = 0; i < objectses.length; i++) {
                query.setParameter(0, objectses[i]);
                query.executeUpdate();
            }
        } catch (Exception e) {
            result = false;
