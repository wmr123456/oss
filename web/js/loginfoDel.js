 ...
 */
Ext.define('js.loginfoDel', {
    extend: 'Ext.grid.Panel',
    initComponent: function () {
        var me = this;
        var store = Ext.create('Ext.data.Store', {
            id: 'loginfoDelPanel', //TODO 分页的ID
            pageSize: 20,
            autoLoad: false,
            proxy: {
                type: 'ajax',
            ...
            listeners: {
                beforeload: function (store, operation) {
                    var name = Ext.getCmp('logpageDelparam1Class');
                    var name1 = Ext.getCmp('logpageDelparam1Value');
                    var name2 = Ext.getCmp('logpageDelparam2Class');
                    var name3 = Ext.getCmp('logpageDelparam2Value');
                    var name4 = Ext.getCmp('logpageDelparam7');
                    var name5 = Ext.getCmp('logpageDelparam8');
                    if (name) {
                        if (name.getValue()) {
                            if (operation.params) {
        ...
        Ext.apply(this, {
            ...
            border: false,
            closable: true,
            title: '删除日志',//TODO 表格的名称
            store: Ext.data.StoreManager.lookup('loginfoDelPanel'), //TODO store的ID
            columns: [
            ...
            dockedItems: [
                {
                    ...
                    items: [
                        {
                            xtype: 'combo',
                            id: 'logpageDelparam1Class',
                            name: 'logpageDelparam1',
                            width: 112,
                            emptyText: '请选择查询方式',
                            emptyValue: '0',
                            store: store2,
                        ...
                        {
                            xtype: 'textfield',
                            id: 'logpageDelparam1Value',
                            name: 'logpageDelparam2'
                        },
                        {
                            xtype: 'combo',
                            id: 'logpageDelparam2Class',
                            name: 'logpageDelparam3',
                            width: 112,
                            emptyText: '请选择查询方式',
                            emptyValue: '0',
                            store: store2,
                        ...
                        {
                            xtype: 'textfield',
                            id: 'logpageDelparam2Value',
                            name: 'logpageDelparam4'
                        },
                        {
                            xtype: 'tbtext',
                            text: '按时间：'
                        ...
                        { xtype: 'button', text: '搜索', border: false, style: {background: '#cbdbef'}, handler: function () {
                            Ext.getCmp('loginfoDelpage').store.load({
                                params: {
                                    param1: Ext.getCmp('logpageDelparam1Class').getValue(),
                                    param2: Ext.getCmp('logpageDelparam1Value').getValue(),
                                    param3: Ext.getCmp('logpageDelparam2Class').getValue(),
                                    param4: Ext.getCmp('logpageDelparam2Value').getValue(),
                                    param7: Ext.getCmp('logpageDelparam7').getValue(),
                                    param8: Ext.getCmp('logpageDelparam8').getValue()
                                }
                            });
                        }
                        }
    ...
    del: function () {
        var me = this;
        var record = Ext.getCmp('loginfoDelpage').getSelectionModel().getSelection();
        var length = record.length;
        var list = '';
        if (length > 1) {
            for (var i = 0; i < length; i++) {
                list += record[i].raw.id;
                if (i != length - 1) {
                    list += ',';
                }
            }
        } else {
            list += record[0].raw.id;
        }
        Ext.Ajax.request({
            url: 'loginfodel',
            method: 'post',
            params: {idList: list},
            success: function () {
                Ext.getCmp('loginfoDelpage').store.reload();
            }
        })
    },
